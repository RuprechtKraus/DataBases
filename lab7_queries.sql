...
ALTER TABLE student
    ADD CONSTRAINT student_group_id_group_fk FOREIGN KEY (id_group)
        REFERENCES `group` (id_group)
        ON UPDATE CASCADE
        ON DELETE CASCADE;

ALTER TABLE mark
    ADD CONSTRAINT mark_lesson_id_lesson_fk FOREIGN KEY (id_lesson)
        REFERENCES lesson (id_lesson)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
    ADD CONSTRAINT mark_student_id_student_fk FOREIGN KEY (id_student)
        REFERENCES student (id_student)
        ON UPDATE CASCADE
        ON DELETE CASCADE;

ALTER TABLE lesson
    ADD CONSTRAINT lesson_teacher_id_teacher_fk FOREIGN KEY (id_teacher)
        REFERENCES teacher (id_teacher)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
    ADD CONSTRAINT lesson_subject_id_subject_fk FOREIGN KEY (id_subject)
        REFERENCES subject (id_subject)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
    ADD CONSTRAINT lesson_group_id_group_fk FOREIGN KEY (id_group)
        REFERENCES `group` (id_group)
        ON UPDATE CASCADE
        ON DELETE CASCADE;
...
SELECT g.name AS `group`, s.name AS student, m.mark
FROM `group` g
         ...
         INNER JOIN subject s2 ON l.id_subject = s2.id_subject AND s2.name = 'Информатика';

SELECT *
FROM computer_science_marks;
...
WITH subjects_with_enough_students (id_subject, name)
         AS
...
SELECT g.name AS `group`, st2.name AS student, swes.name AS subject, AVG(m.mark) AS average_mark
FROM subjects_with_enough_students swes
         ...
         INNER JOIN mark m ON l.id_lesson = m.id_lesson AND m.id_student = st2.id_student
...
SELECT g.name AS `group`, st.name AS student, s.name AS subject, m.mark, l.date
FROM student st
...
UPDATE mark
    INNER JOIN lesson l ON mark.id_lesson = l.id_lesson AND l.date < '2019-05-12' AND l.id_subject = 1
    INNER JOIN student s ON mark.id_student = s.id_student AND s.id_group = 1
SET mark = mark + 1
WHERE mark < 5;
